...
```bash
$ g++ -std=c++14 -o compiler parser.cpp codegen.cpp main.cpp tokens.cpp corefn.cpp `llvm-config --libs` `llvm-config --ldflags` -lpthread -ldl -lz -lncurses -rdynamic
```
...
$ gcc test/example.S -o test/example.native # 再用gcc将汇编代码编译为二进制文件
```
...
```bash
$ llc test/example.bc -o test/example.S # 先将LLVM IR字节码转成汇编代码
$ gcc native.cpp test/example.S -o test/example.native # 再用gcc将汇编代码编译为二进制文件
```
